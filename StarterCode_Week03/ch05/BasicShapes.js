...
// shapes.
  ...
  makeCylinder();
  makeLongCylinder();					// create, fill the cylVerts array
  ...
  // how many floats total needed to store all shapes?
	var mySiz = (cylVerts.length + sphVerts.length + 
							 torVerts.length + gndVerts.length + cylLongVerts.length);						
  ...
  for(i=0,j=0; j< cylVerts.length; i++,j++) {
	...
	longCylStart = i;
	for(j=0; j< cylLongVerts.length; i++, j++) {
		colorShapes[i] = cylLongVerts[j];
	}
...
//
 ...
 // Create a (global) array to hold this cylinder's vertices;
 cylLongVerts = new Float32Array(  ((capVerts*6) -2) * floatsPerVertex);
										// # of vertices * # of elements needed to store them. 
	...
	for(v=1,j=0; v<2*capVerts; v++,j+=floatsPerVertex) {	
		...
		if(v%2==0)
		{				// put even# vertices at center of cylinder's top cap:
			cylLongVerts[j  ] = 0.0; 			// x,y,z,w == 0,0,1,1
			cylLongVerts[j+1] = 0.0;	
			cylLongVerts[j+2] = 1.0; 
			cylLongVerts[j+3] = 1.0;			// r,g,b = topColr[]
			cylLongVerts[j+4]=ctrColr[0]; 
			cylLongVerts[j+5]=ctrColr[1]; 
			cylLongVerts[j+6]=ctrColr[2];
		}
		else { 	// put odd# vertices around the top cap's outer edge;
						// x,y,z,w == cos(theta),sin(theta), 1.0, 1.0
						// 					theta = 2*PI*((v-1)/2)/capVerts = PI*(v-1)/capVerts
			cylLongVerts[j  ] = Math.cos(Math.PI*(v-1)/capVerts);			// x
			cylLongVerts[j+1] = Math.sin(Math.PI*(v-1)/capVerts);			// y
			//	(Why not 2*PI? because 0 < =v < 2*capVerts, so we
			//	 can simplify cos(2*PI * (v-1)/(2*capVerts))
			cylLongVerts[j+2] = 1.0;	// z
			cylLongVerts[j+3] = 1.0;	// w.
			// r,g,b = topColr[]
			cylLongVerts[j+4]=topColr[0]; 
			cylLongVerts[j+5]=topColr[1]; 
			cylLongVerts[j+6]=topColr[2];			
		}
	}
	// Create the cylinder side walls, made of 2*capVerts vertices.
	// v counts vertices within the wall; j continues to count array elements
	for(v=0; v< 2*capVerts; v++, j+=floatsPerVertex) {
		if(v%2==0)	// position all even# vertices along top cap:
		{		
				cylLongVerts[j  ] = Math.cos(Math.PI*(v)/capVerts);		// x
				cylLongVerts[j+1] = Math.sin(Math.PI*(v)/capVerts);		// y
				cylLongVerts[j+2] = 1.0;	// z
				cylLongVerts[j+3] = 1.0;	// w.
				// r,g,b = topColr[]
				cylLongVerts[j+4]=topColr[0]; 
		...
		{
				cylLongVerts[j  ] = botRadius * Math.cos(Math.PI*(v-1)/capVerts);		// x
				cylLongVerts[j+1] = botRadius * Math.sin(Math.PI*(v-1)/capVerts);		// y
				cylLongVerts[j+2] =-1.0;	// z
				cylLongVerts[j+3] = 1.0;	// w.
				// r,g,b = topColr[]
				cylLongVerts[j+4]=botColr[0]; 
				cylLongVerts[j+5]=botColr[1]; 
				cylLongVerts[j+6]=botColr[2];			
		}
	}
	// Create the cylinder bottom cap, made of 2*capVerts -1 vertices.
	// v counts the vertices in the cap; j continues to count array elements
	for(v=0; v < (2*capVerts -1); v++, j+= floatsPerVertex) {
		if(v%2==0) {	// position even #'d vertices around bot cap's outer edge
			cylLongVerts[j  ] = botRadius * Math.cos(Math.PI*(v)/capVerts);		// x
			cylLongVerts[j+1] = botRadius * Math.sin(Math.PI*(v)/capVerts);		// y
			cylLongVerts[j+2] =-1.0;	// z
			cylLongVerts[j+3] = 1.0;	// w.
			// r,g,b = topColr[]
			cylLongVerts[j+4]=botColr[0]; 
			...
			cylLongVerts[j+1] = 0.0;	
			cylLongVerts[j+2] =-1.0; 
			cylLongVerts[j+3] = 1.0;			// r,g,b = botColr[]
			cylLongVerts[j+4]=botColr[0]; 
			cylLongVerts[j+5]=botColr[1]; 
			cylLongVerts[j+6]=botColr[2];
		}
...
//==============================================================================
  ...
  //modelMatrix.setTranslate(currentAngle/200,currentAngle/200, 0.0);  // 'set' means DISCARD old matrix,
  modelMatrix.setTranslate(.4,-.4,0)
  ...
  gl.drawArrays(gl.TRIANGLE_STRIP,
  							longCylStart/floatsPerVertex,
  							cylLongVerts.length/floatsPerVertex);


  //--------Draw Spinning Sphere
  modelMatrix.setTranslate( 0.4, 0.40, 0.2); // 'set' means DISCARD old matrix,
  						// (drawing axes centered in CVV), and then make new
  						// drawing axes moved to the lower-left corner of CVV.
  modelMatrix.scale(1,1,-1);							// convert to left-handed coord sys
  																				// to match WebGL display canvas.
  ...
  modelMatrix.scale(0.3, 0.3, 0.3);
  						// Make it smaller:
  modelMatrix.rotate(currentAngle, 1, 0, 0);  // Spin on YZ axis
	// Drawing:		
	// Pass our current matrix to the vertex shaders:
  gl.uniformMatrix4fv(u_ModelMatrix, false, modelMatrix.elements);
