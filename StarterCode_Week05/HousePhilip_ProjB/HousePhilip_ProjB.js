...
var g_EyeX = -10, g_EyeY = 2, g_EyeZ = 8.25; 
var g_LookAtX = 0, g_LookAtY = -5, g_LookAtZ = 0;
...
function keyup(ev, gl, currentAngle, u_ViewMatrix, viewMatrix, u_ProjMatrix, projMatrix) {
  if (ev.keyCode == 32) {
    press = false;
  }
}
...
// draw this many vertices.
  ...
  myViewMatrix.translate(4, 0 + rocket_height, -7);
  ...
  myViewMatrix.translate(-4, -8 - rocket_height, 7);
 ...
 // old one had "+y points upwards", but
  ...
  myViewMatrix.rotate(-270, 1, 0,0);
  myViewMatrix.translate(-2,3, 0);
  myGL.uniformMatrix4fv(myu_ViewMatrix, false, myViewMatrix.elements);
  ...
  myViewMatrix.rotate(270, 1, 0, 0);
  myViewMatrix.translate(15, 10, 0);
  myGL.uniformMatrix4fv(myu_ViewMatrix, false, myViewMatrix.elements);
  myGL.drawArrays(myGL.TRIANGLES,
                rectanglestart/floatsPerVertex,
                rectangleVerts.length/floatsPerVertex);
  ...
  myViewMatrix.translate(-20, -8, 15);
  ...
  myViewMatrix.translate(Math.sin(toRadians(currentAngle*8))*2, 0, Math.cos(toRadians(currentAngle*8))*2);
